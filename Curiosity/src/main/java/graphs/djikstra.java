...
To update the distance values, iterate through all adjacent vertices. For every adjacent vertex v,
...
public class djikstra {
	public static void main(String[] args) {
		...
		(new djikstra()).findShortestPath(graph, 0);
	}

	private void findShortestPath(int[][] graph, int startVertex) {
		HashSet<Integer> shortesPath = new HashSet<Integer>();
		int max = Integer.MAX_VALUE;
		int[] distance = {0, max, max, max, max, max, max, max, max};
